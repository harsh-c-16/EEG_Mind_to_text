"""Standalone runner: Gaussian Naive Bayes on P300 EEG data.

Runs only Gaussian NB across all subjects and prints results.
...
from eeg_bci.classical import build_gaussian_nb
...
OUTPUT_DIR = "/csehome/b24cm1027/PRML/outputs/classical_gnb"
TRAIN_TRIALS = 12
VAL_TRIALS = 3
...
def main() -> None:
    ...
    models = {"Gaussian NB": build_gaussian_nb()}
    all_results = {"Gaussian NB": {"flash_level": [], "speller_level": []}}
    per_subject = []

    print(f"Running Gaussian NB on {len(files)} subjects...")
    ...
    for filepath in tqdm(files, desc="Subjects"):
        ...
        all_results["Gaussian NB"]["flash_level"].append(subject_results["Gaussian NB"]["flash_level"])
        all_results["Gaussian NB"]["speller_level"].append(subject_results["Gaussian NB"]["speller_level"])

        fl = subject_results["Gaussian NB"]["flash_level"]
        print(
            f"{prepared.subject}: "
    ...
    output_data = {
        "pipeline": "classical_gnb",
        "aggregate_by_model": aggregate_by_model(all_results),
        "per_subject": per_subject,
    }
    output_file = output_dir / "summary_gnb.json"
