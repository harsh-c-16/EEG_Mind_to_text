"""Standalone runner: SVM with RBF kernel on P300 EEG data.

Runs only SVM (RBF) across all subjects and prints results.
...
from eeg_bci.classical import build_svm_rbf
...
OUTPUT_DIR = "/csehome/b24cm1027/PRML/outputs/classical_svm_rbf"
TRAIN_TRIALS = 12
VAL_TRIALS = 3
...
def main() -> None:
    ...
    models = {"SVM (RBF)": build_svm_rbf(random_state=RANDOM_STATE)}
    all_results = {"SVM (RBF)": {"flash_level": [], "speller_level": []}}
    per_subject = []

    print(f"Running SVM (RBF) on {len(files)} subjects...")
    ...
    for filepath in tqdm(files, desc="Subjects"):
        ...
        all_results["SVM (RBF)"]["flash_level"].append(subject_results["SVM (RBF)"]["flash_level"])
        all_results["SVM (RBF)"]["speller_level"].append(subject_results["SVM (RBF)"]["speller_level"])

        fl = subject_results["SVM (RBF)"]["flash_level"]
        print(
            f"{prepared.subject}: "
    ...
    output_data = {
        "pipeline": "classical_svm_rbf",
        "aggregate_by_model": aggregate_by_model(all_results),
        "per_subject": per_subject,
    }
    output_file = output_dir / "summary_svm_rbf.json"
